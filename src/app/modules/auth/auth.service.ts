...
const checkLogin = async (payload: TLogin, req: any) => {
  try {
    ...
    const foundUser = await User.isUserExists(normalizedEmail);
    if (!foundUser) {
      // 1. This throws custom AppError (404)
      throw new AppError(httpStatus.NOT_FOUND, "Login Details are not correct");
    }
    ...
    // 2. Check if the error is already one of your custom AppErrors
    if (error instanceof AppError) {
      throw error; 
    }

    // 3. Only log and wrap unexpected system/database errors here
    console.error("Unexpected error in checkLogin service:", error);
    throw new AppError(
      httpStatus.INTERNAL_SERVER_ERROR,
      "An error occurred during login"
    );
...
const refreshToken = async (token: string) => {
  ...
  // ✅ Wrap verifyToken so expired/invalid tokens are caught properly
  let decoded: any;
  try {
    decoded = verifyToken(token, config.jwt_refresh_secret as string);
  } catch (err) {
    throw new AppError(httpStatus.UNAUTHORIZED, "JWT Expired"); // match frontend check
  }
